      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Tabs defaultValue="basic" className="w-full">
          <TabsList className="grid w-full grid-cols-8">
            ...
            <TabsTrigger value="arabicTranslation">
          ...
          <TabsContent value="arabicTranslation" className="space-y-6 pt-4">
            <div className="border-b pb-4 mb-4">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Provide Arabic translations for the package content
              </p>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="font-medium text-blue-900 mb-1">Smart Auto-Translate</h4>
                  <p className="text-sm text-blue-700">
                    Automatically translate English content to Arabic using Google Gemini AI
                  </p>
                </div>
                <Button
                  type="button"
                  onClick={async () => {
                    try {
                      // Get English content from form
                      const englishFields = {
                        title: form.getValues('title'),
                        shortDescription: form.getValues('shortDescription'),
                        overview: form.getValues('overview'),
                        bestTimeToVisit: form.getValues('bestTimeToVisit'),
                        includedFeatures: form.getValues('includedFeatures'),
                        excludedFeatures: form.getValues('excludedFeatures'),
                        itinerary: form.getValues('itinerary'),
                        whatToPack: form.getValues('whatToPack'),
                        cancellationPolicy: form.getValues('cancellationPolicy'),
                        childrenPolicy: form.getValues('childrenPolicy'),
                        termsAndConditions: form.getValues('termsAndConditions'),
                        customText: form.getValues('customText')
                      };

                      // Filter out empty fields
                      const fieldsToTranslate = Object.entries(englishFields)
                      ...
                      if (fieldsToTranslate.length === 0) {
                        toast({
                          ...
                          description: "Please fill out some English content first",
                          variant: "destructive"
                        });
                        return;
                      }

                      const translations = await geminiService.batchTranslateToArabic(fieldsToTranslate);
                      ...
                      const fieldMapping = {
                        ...
                        overview: 'overviewAr',
                        bestTimeToVisit: 'bestTimeToVisitAr',
                        includedFeatures: 'includedFeaturesAr',
                        excludedFeatures: 'excludedFeaturesAr',
                        itinerary: 'itineraryAr',
                        whatToPack: 'whatToPackAr',
                        cancellationPolicy: 'cancellationPolicyAr',
                        childrenPolicy: 'childrenPolicyAr',
                        termsAndConditions: 'termsAndConditionsAr',
                        customText: 'customTextAr'
                      };

                      translations.forEach(({ field, translatedText }) => {
                        const arabicField = fieldMapping[field];
                        if (arabicField) {
                          form.setValue(arabicField, translatedText);
                        }
                      });

                      toast({
                        title: "Translation completed",
                        description: `Successfully translated ${translations.length} fields to Arabic`,
                      });
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 gap-6">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Package Title (Arabic)</FormLabel>
                      <FormControl>
                        <Input 
                          {...field} 
                          dir="rtl"
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea 
                          {...field} 
                          dir="rtl"
                          placeholder="نظرة عامة على الباقة"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>What to Pack (Arabic)</FormLabel>
                      <FormControl>
                        <Textarea 
                          {...field} 
                          dir="rtl"
                          placeholder="ما يجب إحضاره"
