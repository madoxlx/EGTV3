...
const PackagesSearchResults: React.FC = () => {
  ...
  useEffect(() => {
    localStorage.setItem('packageFavorites', JSON.stringify(Array.from(favorites)));
  }, [favorites]);
  
  // Toggle favorite status
  const toggleFavorite = (packageId: number, packageTitle: string) => {
    const isCurrentlyFavorited = favorites.has(packageId);
    ...
    setTimeout(() => {
      if (isCurrentlyFavorited) {
        toast({
          ...
          description: `${packageTitle} has been removed from your favorites.`,
        });
      } else {
        toast({
          title: "Added to favorites",
          description: `${packageTitle} has been added to your favorites.`,
        });
      }
    }, 0);
  };
                  ...
                  <div className="flex flex-col md:flex-row">
                    ...
                    <div className="hidden md:block md:w-1/3 h-auto relative">
                      ...
                      <div className="absolute top-4 right-4">
                        <button 
                          onClick={() => toggleFavorite(pkg.id, pkg.title)}
