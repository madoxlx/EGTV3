...
const RoomCard = ({ room, onSelect }: { room: any; onSelect: (room: any) => void }) => {
  const amenities = Array.isArray(room.amenities) 
    ? room.amenities 
    : (typeof room.amenities === 'string' 
      ? (room.amenities.startsWith('[') ? JSON.parse(room.amenities) : room.amenities.split(','))
      : []);

  const displayedAmenities = Array.isArray(amenities) ? amenities.slice(0, 5) : [];
    ...
    <Card className="overflow-hidden mb-4">
      <div className="grid grid-cols-1 md:grid-cols-12 gap-0">
        <div className="md:col-span-4 h-[200px] md:h-full">
          <img 
            src={room.imageUrl || 'https://images.unsplash.com/photo-1566665797739-1674de7a421a?q=80&w=2074&auto=format&fit=crop'}
            alt={room.name}
        ...
        <div className="md:col-span-8">
          ...
          <CardContent className="py-2">
            {room.description && (
              <p className="text-sm mb-3 text-muted-foreground">{room.description}</p>
            )}
...
const HotelDetailsPage: React.FC = () => {
    ...
    placeholderData: [
      {
        ...
        size: "32 m²",
        imageUrl: "https://images.unsplash.com/photo-1566665797739-1674de7a421a?q=80&w=2074&auto=format&fit=crop",
        description: "Spacious room with king bed and city view. Includes free WiFi, flat-screen TV, and mini-bar.",
        amenities: ["Free WiFi", "Flat-screen TV", "Air conditioning", "Mini-bar", "Safe", "Hair dryer"]
      },
      {
        id: 2,
        name: "Premium Suite",
        ...
        description: "Luxury suite with separate living area, king bed, and panoramic city views. Includes premium amenities and daily housekeeping.",
        amenities: ["Free WiFi", "Flat-screen TV", "Air conditioning", "Mini-bar", "Safe", "Hair dryer", "Separate living area", "Coffee maker", "Premium toiletries"]
      },
      {
        id: 3,
        name: "Family Room",
        ...
        description: "Spacious room ideal for families with one queen and one twin bed. Features child-friendly amenities and extra space.",
        amenities: ["Free WiFi", "Flat-screen TV", "Air conditioning", "Mini-bar", "Safe", "Hair dryer", "Child-friendly amenities"]
      }
    ]
  });
  ...
  const hotelDisplay = {
    ...
    description: hotel.description || "Experience luxury and comfort in the heart of the city. Our hotel offers elegant rooms, exceptional dining, and world-class amenities for both business and leisure travelers.",
    address: hotel.address || "123 Main Street",
    city: hotel.city || "Cairo",
    country: hotel.country || "Egypt",
    stars: hotel.stars || 4,
    rating: hotel.rating || 4.5,
    imageUrl: hotel.imageUrl || "https://images.unsplash.com/photo-1564501049412-61c2a3083791?q=80&w=2073&auto=format&fit=crop",
    amenities: hotel.amenities || ["Free WiFi", "Swimming Pool", "Restaurant", "Fitness Center", "Spa", "Room Service", "Conference Rooms", "Parking"],
      ...
      <div className="container mx-auto py-8 px-4">
        ...
        <div className="relative rounded-xl overflow-hidden mb-8 h-[400px]">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
            <h1 className="text-3xl md:text-4xl font-bold mb-2">{hotelDisplay.name}</h1>
            <div className="flex items-center mb-2">
              <MapPin size={16} className="mr-1" />
              <span>{hotelDisplay.address || `${hotelDisplay.city}, ${hotelDisplay.country}`}</span>
            </div>
            ...
            <Tabs defaultValue="rooms">
              ...
              <TabsContent value="rooms" className="pt-6">
                <div className="mb-6">
                  ...
                  <p className="text-muted-foreground">
                    Choose from our selection of {rooms.length} room types. Prices shown are per night.
                  </p>
                </div>
              ...
              <TabsContent value="overview" className="pt-6">
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-muted-foreground">{hotelDisplay.description}</p>
                  </div>
                  ...
                  <div>
                    ...
                    {hotelDisplay.phone && (
                      <div className="flex items-start mb-2">
                        <Phone size={18} className="text-primary mr-2 flex-shrink-0 mt-1" />
                        <span>{hotelDisplay.phone}</span>
                      </div>
                    )}
                    {hotelDisplay.email && (
                      <div className="flex items-start mb-2">
                        <Mail size={18} className="text-primary mr-2 flex-shrink-0 mt-1" />
                        <span>{hotelDisplay.email}</span>
                      </div>
                    )}
                    {hotelDisplay.website && (
                      <div className="flex items-start">
                        <Globe size={18} className="text-primary mr-2 flex-shrink-0 mt-1" />
                        <a href={hotelDisplay.website} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">
                          {hotelDisplay.website}
                        </a>
                      </div>
                    )}
                      ...
                      <CardContent className="pt-0">
                        <div className="space-y-3">
                          <div>
                            ...
                            <div className="ml-2 text-muted-foreground">{hotelDisplay.checkInTime}</div>
                          </div>
                          <div>
                            <span className="font-medium">Check-out:</span>
                            <div className="ml-2 text-muted-foreground">{hotelDisplay.checkOutTime}</div>
                          </div>
                          <div>
                            <span className="font-medium">Address:</span>
                            <div className="ml-2 text-muted-foreground">{hotelDisplay.address || `${hotelDisplay.city}, ${hotelDisplay.country}`}</div>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                      ...
                      <CardContent className="pt-0">
                        <ul className="space-y-2">
                          ...
                          {hotelDisplay.parkingAvailable && (
                            <li className="flex items-center">
                              <ParkingIcon size={16} className="text-primary mr-2" />
              ...
              <TabsContent value="reviews" className="pt-6">
                ...
                <div className="bg-muted p-6 rounded-lg mb-6">
                  ...
                  <p className="text-muted-foreground">
                    Great hotel with excellent amenities. The staff was friendly and attentive. Room was clean and comfortable. I would definitely stay here again.
                  </p>
                </div>
                ...
                <div className="bg-muted p-6 rounded-lg">
                  ...
                  <p className="text-muted-foreground">
                    Nice hotel in a great location. The room was a bit smaller than expected, but everything was clean and well-maintained. Breakfast was delicious with many options.
                  </p>
                </div>
              </TabsContent>
            </Tabs>
          ...
          <div id="booking-section">
            <Card className="sticky top-6">
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      <div>
                        ...
                        <select 
                          ...
                          onChange={(e) => setGuests({ ...guests, infants: parseInt(e.target.value) })}
                        >
                          {[0, 1, 2, 3].map(num => (
                            <option key={num} value={num}>{num}</option>
                          ))}
                  ...
                  {selectedRoom ? (
                    <div className="mt-4 p-4 border rounded-md bg-muted">
                      ...
                      <div className="flex justify-between mb-4">
                        <span className="text-sm text-muted-foreground">
                          {selectedRoom.maxAdults} Adults, {selectedRoom.maxChildren} Children
                        </span>
                        <span className="font-bold">
                          ${selectedRoom.discountedPrice || selectedRoom.price} / night
                        </span>
                      </div>
                      <Button variant="ghost" className="w-full text-muted-foreground" onClick={() => setSelectedRoom(null)}>
                        Change Room
                      </Button>
                    </div>
                  ) : (
                    <div className="mt-4 p-4 border rounded-md bg-muted text-center">
                      ...
                      <Button variant="secondary" className="w-full" onClick={() => document.querySelector('[data-value="rooms"]')?.scrollIntoView()}>
                        See Available Rooms
                      </Button>
                    </div>
                  )}
              ...
              <CardFooter className="flex-col space-y-3">
                <Button className="w-full" disabled={!selectedRoom}>Book Now</Button>
